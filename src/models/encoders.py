# Phase 1: Graph-temporal encoders
"""
Temporal graph encoders for micro and macro scales.

The implementation is intentionally pure PyTorch so the repository can run
without optional graph-temporal extensions.
"""
...
def _graph_average(
    ...
    """Average neighbor messages with implicit self-loops."""
...
class TemporalGraphEncoder(nn.Module):
    """Graph-aware recurrent encoder for node-wise temporal sequences."""
    ...
    def __init__(self, in_channels: int, out_channels: int, dropout: float = 0.2):
        ...
        self.message_proj = nn.Linear(out_channels * 2, out_channels)
    ...
    def forward(
        ...
        if x.dim() == 2:
            x = x.unsqueeze(1)
        if x.dim() != 3:
            raise ValueError(f"Expected [N, T, F] or [N, F], got {tuple(x.shape)}")
        ...
        for t in range(x.shape[1]):
            ...
            hidden = self.recurrent(self.dropout(message), hidden)
